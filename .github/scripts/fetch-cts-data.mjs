 ...
 *
 * Estratégia: uma chamada por país (ALL + cada um). Já tentamos um endpoint
 * batch (tudo numa requisição), mas o Apps Script tem teto rígido de 6 min
 * por execução e 7× o trabalho não cabe com a variância de latência dele —
 * então voltamos ao país-por-país, que é lento (~5min) mas robusto: cada
 * chamada é uma execução independente, bem abaixo do teto.
 *
